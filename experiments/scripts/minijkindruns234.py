...
for indx, file in enumerate(models):
    ...
    try:
        tree = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg2_allivcs_inter_loop_runs.xml'))
        ...
        pass
    ivc_info ['adeq_res_alg2'] = adequate
    ivc_info ['inadeq_res_alg2'] = inadequate
    ivc_info.close() 
...
for indx, file in enumerate(models):
    ...
    n = 0
    try: 
        tree = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg2_all_uc_minijkind.xml')) 
    ...
    if (n <= 1):
        print (file)
    ivc_info ['discovered_mivcs_alg2'] = n
    ivc_info.close()
