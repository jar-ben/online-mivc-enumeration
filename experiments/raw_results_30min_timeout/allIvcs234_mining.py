...
for file in models:
    try:
        tree = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg3_uc.xml'))
        proof = 0.0
    ...
    try:
        tree2 = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg3_runtimeAllIvcs.xml'))
...
timing_info ['all_ivcs_no_proof_alg3'] = all_ivcs_time
timing_info ['proof_time_alg3'] = proof_time
timing_info ['uc_time_no_proof_alg3'] = uc_time
timing_info ['uc_time_w_proof_alg3'] = uc_w_proof
timing_info ['all_ivcs_w_proof_alg3'] = all_ivcs_w_proof
timing_info ['num_of_ivc_calls_alg3'] = numofivccalls
timing_info ['timedout_alg3'] = timedout
...
for i, file in enumerate(models): 
    ...
    try:
        ivc_info ['number_of_ivc_sets_alg3'] = unm_of_ivcs[i]
    except:
        ivc_info ['number_of_ivc_sets_alg3'] =  'nan'
        print(file)
        print("unknown") 
    id = 0
    min = 10000
    for elem in tree.iter(tag = 'IvcSet'): 
        ...
        ivc_info ['alg3_set' + str(id)] = ivc_set
        id += 1
        if len(ivc_set) < min:
            minimal = list(ivc_set)
            min = len(ivc_set)
    ivc_info ['minimum_alg3'] = minimal   
    ivc_info.close() 
    
timing_info['number_of_ivc_sets_alg3'] = unm_of_ivcs    '''
timing_info.close()
