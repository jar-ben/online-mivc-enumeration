...
for file in lus_files: 
    try:
        tree = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg2_uc.xml'))
...
for file in sorted_models_mem:
    ...
    try:
        tree2 = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg2_runtimeAllIvcs.xml'))
...
timing_info ['all_ivcs_no_proof_alg2'] = all_ivcs_time
timing_info ['proof_time_alg2'] = proof_time
timing_info ['uc_time_no_proof_alg2'] = uc_time
timing_info ['uc_time_w_proof_alg2'] = uc_w_proof
timing_info ['all_ivcs_w_proof_alg2'] = all_ivcs_w_proof
timing_info ['num_of_ivc_calls_alg2'] = numofivccalls
timing_info ['timedout_alg2'] = timedout
...
for i, file in enumerate(sorted_models_mem): 
    ...
    try:
        ivc_info ['number_of_ivc_sets_alg2'] = unm_of_ivcs[i]
    except:
        ivc_info ['number_of_ivc_sets_alg2'] =  'nan'
        print(file)
        print("unknown") 
    id = 0
    min = 10000
    for elem in tree.iter(tag = 'IvcSet'): 
        ...
        ivc_info ['alg2_set' + str(id)] = ivc_set
        id += 1
        if len(ivc_set) < min:
            minimal = list(ivc_set)
            min = len(ivc_set)
    ivc_info ['minimum_alg2'] = minimal  
    /    ivcuc=[]
    treeuc = ET.ElementTree(file = os.path.join(RESULTS_DIR, file + '_alg2_uc.xml'))
    for ivc in treeuc.iter(tag = 'TRIVC'):
        ivcuc.append(ivc.text)
    ivc_info ['uc_input_for_ucbf_alg2'] = ivcuc/
    ivc_info.close() 
    
timing_info['number_of_ivc_sets_alg2'] = unm_of_ivcs   ''' 
timing_info.close()
