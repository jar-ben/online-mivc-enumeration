...
A straightforward way to find a (so far unexplored) MIVC of $T$ is to find an unexplored adequate subset $U \subseteq T$ and turn $U$  into an MIVC by a process called \emph{shrinking}. It iteratively attempts to remove elements from the set that is being shrunk, checking each new set for adequacy and keeping only changes that
leave the set adequate (see Algorithm~\ref{shrin-procedure} for pseudocode). To find an unexplored adequate subset Ghassabani et al.~\cite{}  always choose maximal unexplored subset adn test it for adequacy.
